...
var heartBeat = function() {
    if ( kafkaProducer != null) {
        try {
            ...
            kafkaProducer.send([
                {
                    topic: kafka_credentials.topics.heartbeat.name,
                    partition: 0,
...
exports.start = function () {
    if ( kafkaProducer === null ) {
        var kafkaClient;
        try {
            kafkaClient = new Kafka.KafkaClient({kafkaHost: kafka_credentials.hosts});
        } catch (exception) {
    ...
    heartBeatInterval = setInterval( function () {
        heartBeat();
    }, parseInt(kafka_credentials.topics.heartbeat.interval))
};
